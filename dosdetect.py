...
class dos_attack():
	...
	def detect(self,event):
		...
		for m in mac:
			...
			#print(list(dict.fromkeys([src[i] for i, x in enumerate(mac) if x == m])))	
		prev_event = [event[mac.index(i)] for i in [mac_dic.keys()[mac_dic.values().index(x)] for x in mac_dic.values() if x>=1]];#print(prev_event)
		if prev_event:
				print('#################### DDOS ATTACK DETECTED ( SOURCE ) #########################')
			...
			if prev_event:
				print('#################### DDOS ATTACK DETECTED ( DESTINATION ) #########################')
				self.prevent(prev_event)

	def prevent(self,event):
		for i in event:
			print('MITIGATING DOS ATTACK')
			print('Deleting Old Entries & Updating')
